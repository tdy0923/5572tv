...
5572影视 提供完整的 TVBox 配置接口，将您的视频源无缝导入到 TVBox 应用中。支持多种配置模式、智能 spider jar 管理、安全访问控制等高级功能。

## 🚀 快速开始

### 1. 访问配置页面

在 5572影视 网站中，点击导航栏中的“TVBox 配置”菜单，或直接访问：
...
5572影视 支持两种 Token 认证模式：**全局 Token** 和 **用户专属 Token**。
...
**仅适用于部署在 Vercel 的用户**，5572影视 自动启用全球 CDN 加速：
...
5572影视 的 Cron 任务（每天凌晨 1 点）会自动执行以下操作：
...
5572影视 默认启用智能搜索代理，解决 TVBox 搜索结果不精确的问题。
...
5572影视 支持通过 Cloudflare Worker 为视频源 API 提供全球 CDN 加速。
...
5572影视 自动管理 spider jar 文件，确保最佳可用性：
...
- [5572影视 当前维护仓库](https://github.com/tdy0923/5572tv)
