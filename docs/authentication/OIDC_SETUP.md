...
本文档详细介绍如何在 5572影视 中配置 OIDC 单点登录（SSO），支持 Google、Microsoft、GitHub、Facebook、微信、Apple、LinuxDo 等主流身份提供商。
...
- 生产环境：`https://5572tv.example.com/api/auth/oidc/callback`
...
- **Application name**：`5572TV`
- **Homepage URL**：`https://your-domain.com`
- **Application description**（可选）：`5572影视平台`
...
| 特性                 | 标准 OIDC              | GitHub OAuth            | 5572影视 处理                   |
| -------------------- | ---------------------- | ----------------------- | ------------------------------- |
...
> 📝 **隐私说明**：如果用户未公开邮箱，5572影视 会自动从 `/user/emails` 端点获取 primary verified email（需要 `user:email` scope）。
...
Facebook 提供 OAuth 2.0 认证服务，拥有全球最大的用户基数。5572影视 已针对 Facebook 的特殊实现进行了适配。
...
4. 填写应用信息：
   - **App Name**（应用名称）：输入您的应用名称（例如：5572TV）
...
### 5572影视 后台配置（Facebook）

在 5572影视 管理后台 → **系统设置** → **OIDC 认证配置** 中：
...
Facebook 使用 OAuth 2.0 协议，与标准 OIDC 有以下差异（5572影视 已自动处理）：

| 差异项            | 标准 OIDC        | Facebook OAuth         | 5572影视 处理 |
| ----------------- | ---------------- | ---------------------- | ------------- |
...
> 📝 **说明**：Facebook 用户可以选择不分享邮箱，5572影视 使用 `id` 字段作为唯一标识，不强制要求邮箱。
...
1. 检查 5572影视 配置中的 **"Provider ID"** 字段
...
- 5572影视 使用 Facebook 的唯一 ID（`id` 字段）作为用户标识，不强制要求邮箱
...
微信使用 OAuth 2.0 协议，与标准 OIDC 有以下差异（5572影视 已自动处理）：

| 差异项                   | 标准 OIDC              | 微信 OAuth                         | 5572影视 处理 |
| ------------------------ | ---------------------- | ---------------------------------- | ------------- |
...
> 📝 **说明**：微信不一定返回邮箱，5572影视 使用 `openid` 作为唯一标识。
...
2. 只填写域名（如 `5572tv.example.com`），不要加协议或路径
3. 确保域名与实际访问域名完全一致
...
5. 选择类型：**App**
6. 填写信息：
   - **Description**：`5572TV App`
   - **Bundle ID**：`com.yourcompany.5572tv`
...
3. 填写信息：
   - **Description**：`5572TV Web Login`
   - **Identifier**：`com.yourcompany.5572tv.web`（不同于 App ID）
...
1. 进入 **Keys** → 点击 **+** 创建
2. **Key Name**：`5572TV Sign in with Apple Key`
...
const token = jwt.sign({}, privateKey, {
  ...
  subject: 'com.yourcompany.5572tv.web', // 你的 Services ID
...
| **Client ID**              | `com.yourcompany.5572tv.web`               | 你的 Services ID                                       |
...
```json
{
  ...
  "clientId": "com.yourcompany.5572tv.web",
...
| 特性                  | 标准 OIDC           | Apple Sign In                 | 5572影视 处理        |
| --------------------- | ------------------- | ----------------------------- | -------------------- |
...
1. **response_mode=form_post**：
   ...
   - 5572影视 添加了 POST handler 专门处理 Apple 回调

2. **id_token 解析**：
   - Apple 不提供 UserInfo Endpoint
   - 用户信息在 id_token（JWT）中
   - 5572影视 自动解析 JWT payload 获取用户信息
...
- Apple 允许用户隐藏真实邮箱
- 5572影视 使用 `sub` 字段作为唯一标识，不强制要求邮箱
- 如果需要邮箱，可以在首次注册时要求用户补充
...
3. 填写应用注册表单：
   ...
   | **Software ID**      | 软件包标识符（可选）         | `com.yourcompany.5572tv`                         |
...
**在 5572影视 中配置最低信任等级**（`minTrustLevel` 字段）：
...
1. 登录 5572影视 管理后台：`https://your-domain.com/admin`
2. 滚动到 **OIDC 登录配置** 部分
3. 点击配置卡片展开设置
...
2. 查看 5572影视 后台日志（浏览器控制台 Network 标签）
...
1. 保存 OIDC 配置后，退出 5572影视 登录
2. 访问登录页面，应该看到 OIDC 登录按钮
3. 点击按钮，应跳转到提供商登录页面
4. 输入账号密码，授权后应自动跳回 5572影视
5. 检查是否成功登录，用户名显示正确
...
**✅ 已支持**！5572影视 的多 Provider 模式允许同时配置多个 OIDC 提供商。
...
- OIDC 用户没有传统密码
- 用户通过 OIDC 提供商（如 Google）登录，5572影视 不存储密码
...
1. 检查 5572影视 后台日志
2. 查看浏览器控制台错误信息
3. 提交 Issue 到 [5572tv GitHub 仓库](https://github.com/tdy0923/5572tv)
