...
services:
  5572tv-core:
    image: ghcr.io/tdy0923/5572tv:latest
    container_name: 5572tv-core
    restart: on-failure
    ports:
      - '3000:3000'
    environment:
      ...
      - KVROCKS_URL=redis://5572tv-kvrocks:6666
...
services:
  5572tv-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://5572tv-redis:6379
    networks:
      - 5572tv-network
    depends_on:
      - 5572tv-redis
...
# 创建项目目录
mkdir -p /volume1/docker/5572tv
cd /volume1/docker/5572tv
...
4. **创建 Dockerfile**
   ...
   ```dockerfile
   FROM ghcr.io/tdy0923/5572tv:latest
   ```
...
6. **等待部署完成**
   ...
   - 部署完成后，访问 `https://huggingface.co/spaces/你的用户名/5572tv`
...
└── Dockerfile     # FROM ghcr.io/tdy0923/5572tv:latest
```
