...
let ongoingWatchingUpdatesCheck: Promise<void> | null = null;
let lastWatchingUpdatesFetchAt = 0;
...
export async function checkWatchingUpdates(
  ...
  if (!forceRefresh && ongoingWatchingUpdatesCheck) {
    return ongoingWatchingUpdatesCheck;
  }

  const now = Date.now();
  if (
    !forceRefresh &&
    now - lastWatchingUpdatesFetchAt < WATCHING_UPDATES_FETCH_DEDUP_MS
  ) {
    return ongoingWatchingUpdatesCheck ?? Promise.resolve();
  }

  lastWatchingUpdatesFetchAt = now;

  ongoingWatchingUpdatesCheck = (async () => {
    try {
      ...
      try {
        ...
        const newReleases = Object.entries(reminders)
          .filter(([key, reminder]) => {
            ...
            // 检查是否已经在播放记录中（避免重复）
            const isInPlayRecords = records.some(
              (r) => r.title === reminder.title && r.year === reminder.year,
            );
