...
/**
 * 简单的 Prompt 注入防护：剥离危险指令关键词并截断过长输入
 */
function sanitizeForPrompt(input: string, maxLength = 1000): string {
  ...
  const injectionPatterns = [
    /忽略\s*(指令|上面的|之前的|系统)/gi,
    /ignore\s*(instructions|above|previous|system)/gi,
    /你(是|现在是|扮演)/gi,
    /you\s+(are|now\s+are|will\s+act)/gi,
    /请(输出|显示|打印)(你的|系统)?(提示词|prompt|指令)/gi,
