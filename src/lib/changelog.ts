...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: '2026-04-16',
    added: [
      '🆕 独立项目首发：以 `5572tv / 5572影视` 作为独立仓库、独立品牌与独立发布主线正式发布',
      '🛡️ 非商用使用声明：README / README_EN 明确补充仅供学习研究、禁止商用与第三方内容版权责任说明',
    ],
    changed: [
      '🏷️ 独立品牌迁移：README、文档、版本面板、版本检查、Render 配置、Docker / GitHub Actions 部署链路全面切换到 `5572tv`',
      '🚢 部署主链路切换：线上主容器与网络切换为 `5572tv-core` / `5572tv-net`，镜像主线切换至 `ghcr.io/tdy0923/5572tv`',
      '💾 运行时命名迁移：浏览器缓存、播放相关 key 与部分协议参数逐步迁移到 `5572tv_*`，同时保留旧 key 兼容读取',
      '⚡ 全站加速优化：图片代理、Douban 只读接口、Bangumi、短剧推荐、壁纸接口纳入更合理的缓存和 Cloudflare 边缘缓存策略',
    ],
    fixed: [
      '🔐 修复认证链路一致性：真实验证桌面/移动端登录与注册提交流程，修复公开注册后后台删除用户的识别一致性问题',
      '🎬 修复播放页失效线路治理：详情 404 降噪、失效源从线路列表剔除、临时失效 TTL 恢复、严格元信息恢复与更准确错误提示',
      '🖼️ 修复图片 Mixed Content：公共图片 URL 统一规范化为 HTTPS，减少浏览器 Mixed Content 噪音',
      '📺 修复预告片/视频代理噪音：收敛 WatchRoom 与 trailer / video-proxy 失败路径日志，减少无意义重试和红字输出',
    ],
  },
  {
    version: '6.3.1',
    date: '2026-04-14',
    added: [
      '🎨 5572 品牌资源：新增 5572影视 / 5572TV 品牌 Logo、数字 5 图标、favicon 与 PWA 图标资源',
      '🏷️ 仓库品牌化：补充当前仓库定位、演示站信息与项目来源说明，完善 Fork 来源与相关项目致谢',
    ],
    changed: [
      '🏷️ 品牌默认值统一：中文默认品牌统一为 5572影视，英文品牌统一为 5572TV，部署文档与示例环境变量同步更新',
      '📦 仓库元数据整理：版本检查、远程 changelog、部署按钮与文档链接统一指向当前维护仓库 `tdy0923/LunaTV`',
      '🧹 控制台噪音治理：收敛首页、UserMenu、watching-updates、HeroBanner、豆瓣缓存命中等高频调试日志',
    ],
    fixed: [
      '📅 修复即将上映 Worker 日期处理：修复 `Invalid time value` 导致的发布日历处理失败',
      '🔄 修复追更与菜单重复触发：为 watching-updates 增加去重与空值保护，减少切页时的重复请求和重复渲染',
      '🎬 修复 HeroBanner 预告片重试过度：限制 trailer 刷新范围与频率，为失败刷新增加本地退避和服务端负缓存',
      '🖼️ 优化豆瓣图片代理稳定性：为 doubanio 图片增加候选源回退、内容类型校验与更准确的 502 上游错误语义',
      '🔍 优化详情与搜索链路：为详情接口增加保守短缓存，为慢/坏搜索源增加短期熔断，提升弱配置服务器稳定性',
      '🎞️ 减少首页无效短剧详情请求：移除首页短剧详情预抓取，降低切换页面时的 400 请求噪音',
    ],
  },
  {
    version: '6.3.0',
