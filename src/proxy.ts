...
function shouldSkipAuth(pathname: string): boolean {
  const skipPaths = [
    ...
    '/icon-5.svg',
    '/screenshot.png',
    '/poster-cache/', // 海报本地缓存（静态文件，公共访问）
    '/video-cache/', // 视频缩略图缓存（静态文件，公共访问）
...
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico|login|register|oidc-register|warning|download|poster-cache/|video-cache/|api/login|api/register|api/logout|api/cron|api/server-config|api/tvbox|api/live/merged|api/parse|api/bing-wallpaper|api/proxy/|api/telegram/|api/auth/oidc/|api/watch-room/|offline\\.html|sw\\.js).*)',
  ],
};
