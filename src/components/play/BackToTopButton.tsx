...
const BackToTopButton = memo(function BackToTopButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`fixed bottom-[calc(16px+env(safe-area-inset-bottom))] right-4 sm:bottom-6 sm:right-6 z-70 w-12 h-12 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 ease-in-out flex items-center justify-center group relative overflow-hidden ${
        show
