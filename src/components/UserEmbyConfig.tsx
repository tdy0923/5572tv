...
export const UserEmbyConfig = memo(({ initialConfig }: UserEmbyConfigProps) => {
  ...
  const handleEdit = (index: number) => {
    ...
    setTimeout(() => {
      ...
      if (refServerURL.current)
        refServerURL.current.value = source.ServerURL || '';
      if (refApiKey.current) refApiKey.current.value = source.ApiKey || '';
      if (refUsername.current)
        refUsername.current.value = source.Username || '';
      ...
      {publicSources.length > 0 && !showAddForm && editingIndex === null && (
        <div className='space-y-2'>
          <div className='flex items-center gap-2 text-xs font-medium text-purple-700 dark:text-purple-400 uppercase tracking-wide'>
            ...
            <span>公共源</span>
            <span className='text-purple-400 dark:text-purple-600 normal-case font-normal'>
              由管理员提供，自动可用
            </span>
          </div>
          {publicSources.map((source) => (
            <div
              key={source.key}
              className='p-3 border border-purple-200 dark:border-purple-800 rounded-lg bg-purple-50/50 dark:bg-purple-900/10'
            >
              <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='px-1.5 py-0.5 text-xs bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300 rounded'>
                    公共
                  </span>
                </div>
            ...
            <div className='mt-2 space-y-2 pl-2 border-l-2 border-gray-200 dark:border-gray-700'>
              {[
                ...
                <label key={key} className='flex items-center gap-2 text-xs'>
                  <input
                    type='checkbox'
                    checked={formChecks[key as keyof typeof formChecks]}
                    onChange={(e) =>
                      setFormChecks((prev) => ({
                        ...prev,
