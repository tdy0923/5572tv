...
const EpisodeSelector: React.FC<EpisodeSelectorProps> = ({
  ...
  return (
    <div className='md:ml-2 px-4 sm:px-4 py-0 h-full min-h-0 rounded-xl bg-black/10 dark:bg-gray-800 flex flex-col border border-white/0 dark:border-white/30 overflow-hidden'>
      {/* 主要的 Tab 切换 - 美化版本 */}
      ...
      {activeTab === 'episodes' && (
        <>
          ...
          <div className='flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4 border-b border-gray-300 dark:border-gray-700 -mx-4 px-4 shrink-0'>
            ...
            {/* 向上/向下按钮 */}
            <button
              className='shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center text-gray-700 hover:text-green-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-green-400 dark:hover:bg-white/20 transition-all duration-200  active:scale-95 transform translate-y-[-4px]'
              onClick={() => {
                // 切换集数排序（正序/倒序）
                  ...
                  className={`group min-h-[44px] min-w-[44px] px-2 sm:px-3 py-2 flex items-center justify-center text-xs sm:text-sm font-semibold rounded-lg transition-all duration-200 whitespace-nowrap font-mono relative overflow-hidden active:scale-95
                    ${
                      isActive
                        ...
                        : 'bg-linear-to-r from-gray-200 to-gray-100 text-gray-700 hover:from-gray-300 hover:to-gray-200  hover:shadow-md dark:from-white/10 dark:to-white/5 dark:text-gray-300 dark:hover:from-white/20 dark:hover:to-white/15'
                    }`.trim()}
                >
                  {/* 激活态光晕效果 */}
                  {isActive && (
                        ...
                        {isCurrentSource && (
                          <div className='absolute top-2 right-2 z-10'>
                            <div className='relative'>
                              <div className='absolute inset-0 bg-green-500 rounded-full blur opacity-60 animate-[fluent2-shimmer_1.5s_ease-in-out_infinite]'></div>
