    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <section className="relative min-h-screen flex items-center bg-black">
        <div className="absolute inset-0">
          <Image src="/images/agnes/epic-bg.png" alt="" fill className="object-cover opacity-30" priority />
          ...
          <div className={`flex flex-col lg:flex-row items-center gap-12 lg:gap-20 transition-all duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex-1 text-center lg:text-left">
              <div className="flex items-center gap-3 mb-6 justify-center lg:justify-start">
                <Image src="/icons/icon-192x192.png" alt="5572" width={48} height={48} className="rounded-xl" />
                <div>
              ...
              <div className="flex gap-2 mb-6 justify-center lg:justify-start">
                {(['android', 'ios', 'tv'] as const).map((tab) => (
                  ...
                  >
                    {tab === 'android' ? <Smartphone className="w-4 h-4" /> : tab === 'ios' ? <Smartphone className="w-4 h-4" /> : <Tv className="w-4 h-4" />}
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
                {selectedPlatform === 'ios' ? (
                  <button onClick={() => setShowGuide(true)} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#f4c24d] text-black rounded-xl font-semibold hover:bg-[#d89c18] transition-colors min-h-[56px]">
                    添加到主屏幕
                  </button>
                ) : (
                  <a href="/download/5572tv-android.apk?v=1.5.0" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#f4c24d] text-black rounded-xl font-semibold hover:bg-[#d89c18] transition-colors min-h-[56px]">
                    <Download className="w-5 h-5" />
                    下载 {selectedPlatform === 'android' ? 'Android' : 'TV'}
                  </a>
                )}
                <Link href="/" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-white/10 text-white rounded-xl font-semibold hover:bg-white/20 transition-colors min-h-[56px]">
                  网页版体验
                </Link>
              </div>
              <p className="text-xs text-gray-500 mt-4 justify-center lg:justify-start flex">v1.5.0 · 65MB</p>
            </div>
      ...
      <section className="py-20 px-6 sm:px-12 lg:px-20 bg-[#111]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f, i) => (
              <div key={i} className="relative p-6 rounded-2xl bg-white/[0.03] border border-white/5 overflow-hidden group hover:border-[#f4c24d]/30 transition-all duration-300 hover:scale-105">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                {(selectedPlatform === 'ios' ? [
                  ...
                  <div key={i} className="flex items-center gap-4">
                    <span className="w-8 h-8 rounded-full bg-[#f4c24d]/20 text-[#f4c24d] flex items-center justify-center text-sm font-bold flex-shrink-0">{i + 1}</span>
            ...
            <div>
              ...
              <div className="space-y-3">
                {['多源聚合播放', 'AI智能搜索推荐', '弹幕互动', '离线缓存', '多端同步'].map((feature, i) => (
      ...
      {showGuide && <InstallGuide platform={selectedPlatform} onClose={() => setShowGuide(false)} />}
    </div>
  );
}
