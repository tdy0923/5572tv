...
function getAllowedUserApiKeys(
  ...
  if (user.tags && user.tags.length > 0 && config.UserConfig.Tags) {
    ...
    user.tags.forEach((tagName) => {
      ...
      if (tagConfig?.enabledApis) {
        tagConfig.enabledApis.forEach((apiKey) => inheritedApis.add(apiKey));
      }
    });
...
function pruneUserPermissionsByGroups(
  config: Awaited<ReturnType<typeof getConfig>>,
  user: (typeof config.UserConfig.Users)[number],
): void {
  const allowedApiKeys = getAllowedUserApiKeys(config, user);

  // 没有用户组限制时，保留用户当前的个性化配置
  if (allowedApiKeys.length === 0) {
    return;
  }

  const allowedSet = new Set(allowedApiKeys);

  if (user.enabledApis && user.enabledApis.length > 0) {
    user.enabledApis = user.enabledApis.filter((apiKey) =>
      allowedSet.has(apiKey),
    );
  ...
  if (user.tvboxEnabledSources && user.tvboxEnabledSources.length > 0) {
    user.tvboxEnabledSources = user.tvboxEnabledSources.filter((apiKey) =>
      allowedSet.has(apiKey),
    );
    if (user.tvboxEnabledSources.length === 0) {
      delete user.tvboxEnabledSources;
    }
  }
}
    ...
    switch (action) {
      ...
      case 'updateUserApis': {
        ...
        if (enabledApis && enabledApis.length > 0) {
          const allowedApiKeys = getAllowedUserApiKeys(
            adminConfig,
            targetEntry,
          );

          // 如果用户组未限制，则允许自由配置；否则只能在用户组允许范围内收窄
          if (allowedApiKeys.length === 0) {
            targetEntry.enabledApis = enabledApis;
          } else {
            const allowedSet = new Set(allowedApiKeys);
            targetEntry.enabledApis = enabledApis.filter((apiKey) =>
              allowedSet.has(apiKey),
            );
          }
        } else {
          ...
          case 'delete': {
            ...
            adminConfig.UserConfig.Users.forEach((user) => {
              if (user.tags && user.tags.includes(groupName)) {
                ...
                pruneUserPermissionsByGroups(adminConfig, user);
              }
            });
      ...
      case 'updateUserGroups': {
        ...
        pruneUserPermissionsByGroups(adminConfig, targetEntry);
      ...
      case 'batchUpdateUserGroups': {
        ...
        for (const targetUsername of usernames) {
          ...
          if (targetUser) {
            ...
            pruneUserPermissionsByGroups(adminConfig, targetUser);
          }
        }
